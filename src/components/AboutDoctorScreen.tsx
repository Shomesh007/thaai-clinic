...
export const AboutDoctorScreen: React.FC<AboutDoctorScreenProps> = ({
  ...
  return (
    <article
      ...
      className="flex-1 overflow-y-auto pb-24 bg-slate-50/50 text-gray-800 selection:bg-pink-100 selection:text-pink-600"
    >
      ...
      <HeaderNav
        title="About Doctor"
        subtitle="Dr. Sakthimaindan Karthigeyan"
        onBack={() => setActiveTab('home')}
        rightAction={
          onOpenNotifications ? (
            <button
              onClick={onOpenNotifications}
              className="relative p-2 rounded-full hover:bg-pink-50 text-pink-600 transition-colors cursor-pointer"
      ...
      {/* ── MAIN CONTENT ──────────────────────────────────────────────── */}
      <div className="px-5 pt-4 space-y-5 max-w-[430px] mx-auto">

        {/* ── HERO PROFILE CARD ───────────────────────────────────────── */}
        <section className="bg-white rounded-3xl p-5 border border-gray-100 shadow-2xs space-y-4">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4">
            ...
            <div className="relative shrink-0">
              ...
              {/* Available indicator */}
              <span className="absolute -bottom-1 -right-1 bg-emerald-500 border-[3px] border-white text-white text-[9px] font-extrabold px-2 py-0.5 rounded-full shadow-xs">
                Available
              </span>
            </div>
            ...
            <div className="flex-1 text-center sm:text-left space-y-1.5">
              <h2
                className="text-xl sm:text-2xl font-extrabold text-gray-900 tracking-tight leading-tight"
                itemProp="name"
              >
                Dr. Sakthimaindan Karthigeyan
              </h2>

              <p className="text-xs sm:text-sm font-bold text-pink-600 leading-snug">
              ...
              <div className="flex flex-wrap items-center justify-center sm:justify-start gap-1.5 pt-0.5">
                {['MBBS', 'Fellowship – Diabetes', 'ACDM UK'].map(tag => (
                  <span
                    key={tag}
                    className="text-[10px] font-extrabold text-pink-600 bg-pink-50 border border-pink-100 px-2.5 py-0.5 rounded-full"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <p
                className="text-xs text-gray-600 font-medium leading-relaxed pt-1"
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-block text-[11px] font-bold text-[#0A66C2] hover:underline pt-0.5"
              >
        ...
        {/* 1. CORE PILLARS */}
        <section aria-label="Core Practice Values" className="bg-white rounded-3xl p-4 border border-gray-100 shadow-2xs">
          <div className="grid grid-cols-4 divide-x divide-pink-100 text-center">
            {[
              { icon: <Heart className="w-4 h-4 fill-pink-600 stroke-pink-600" />, title: 'Patient First', sub: 'Your health first' },
              ...
              <div key={title} className="px-1 flex flex-col items-center">
                <div className="w-8 h-8 rounded-full bg-pink-50 border border-pink-100 text-pink-600 flex items-center justify-center mb-1.5 shadow-2xs">
                  {icon}
                </div>
                <h3 className="text-[10px] font-bold text-pink-600 leading-tight">{title}</h3>
        ...
        >
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-pink-50 border border-pink-100 text-pink-600 flex items-center justify-center shrink-0">
          ...
          <div className="relative pl-4 space-y-3">
            ...
            {QUALIFICATIONS.map((q, i) => (
              <div key={i} className="relative pl-4">
                <span className="absolute left-[-13px] top-1 w-2.5 h-2.5 rounded-full bg-pink-600 ring-2 ring-pink-50"></span>
                <h4 className="font-extrabold text-gray-900 text-xs leading-snug">{q.degree}</h4>
                <p className="text-[10px] text-pink-600 font-semibold mt-0.5">{q.institution}</p>
        ...
        <section
          ...
          className="bg-white rounded-3xl p-4 sm:p-5 border border-gray-100 shadow-2xs"
        >
          <button
          ...
          >
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-pink-50 border border-pink-100 text-pink-600 flex items-center justify-center shrink-0 group-hover:bg-pink-100 transition-colors">
                <Briefcase className="w-3.5 h-3.5 stroke-[2]" />
              </div>
              <div>
                <h3 className="text-sm font-extrabold text-gray-900 tracking-tight group-hover:text-pink-600 transition-colors">
                  Clinical Experience
                </h3>
            ...
            {showExperience
              ...
              : <ChevronRight className="w-4 h-4 text-pink-600 transition-transform duration-200" />
            }
          </button>

          {showExperience && (
            <div className="relative pl-4 space-y-3 pt-3 mt-3 border-t border-gray-100">
              <div className="absolute left-[7px] top-5 bottom-3 w-[1.5px] bg-pink-200"></div>
              {EXPERIENCE.map((e, i) => (
                <div key={i} className="relative pl-4">
                  <span className="absolute left-[-13px] top-1 w-2.5 h-2.5 rounded-full bg-pink-600 ring-2 ring-pink-50"></span>
                  <h4 className="font-extrabold text-gray-900 text-xs">{e.role}</h4>
                  <p className="text-[10px] text-pink-600 font-semibold mt-0.5">{e.place}</p>
        ...
        {/* 4. AREAS OF CARE */}
        <section aria-label="Areas of Care" className="bg-white rounded-3xl p-4 sm:p-5 border border-gray-100 shadow-2xs">
          <button
            onClick={() => setShowAreasOfCare(!showAreasOfCare)}
            className="w-full flex items-center justify-between text-left cursor-pointer group"
          >
            <div className="flex items-center gap-3">
              <div className="w-8 h-8 rounded-full bg-pink-50 border border-pink-100 text-pink-600 flex items-center justify-center shrink-0 group-hover:bg-pink-100 transition-colors">
            ...
            <ChevronRight className={`w-5 h-5 text-pink-600 transition-transform duration-200 ${showAreasOfCare ? 'rotate-90' : ''}`} />
          </button>

          {showAreasOfCare && (
            <div className="grid grid-cols-3 gap-2 pt-3 mt-3 border-t border-gray-100">
              {[
                { img: childHealthIcon, label: 'Child Health', bg: 'bg-pink-50/60 border-pink-100' },
                { img: diabetesCareIcon, label: 'Diabetes Care', bg: 'bg-blue-50/60 border-blue-100' },
                { img: respiratoryCareIcon, label: 'Pulmonary Care', bg: 'bg-emerald-50/60 border-emerald-100' },
                { img: generalConsultationIcon, label: 'General Consultation', bg: 'bg-purple-50/60 border-purple-100' },
                { img: personalisedAttentionIcon, label: 'Diet & Weight', bg: 'bg-amber-50/60 border-amber-100' },
                { img: careForAllAgesIcon, label: 'Preventive Care', bg: 'bg-indigo-50/60 border-indigo-100' },
              ].map(({ img, label, bg }) => (
        ...
        <section
          ...
          className="bg-white rounded-3xl p-4 sm:p-5 border border-gray-100 shadow-2xs space-y-4"
        >
          ...
          {/* Clinic name header */}
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-pink-50 border border-pink-100 text-pink-600 flex items-center justify-center shrink-0 shadow-2xs">
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-xs font-bold text-pink-600">Address</h4>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h4 className="text-xs font-bold text-pink-600">Phone</h4>
              <a
                href="tel:+918610448427"
                itemProp="telephone"
                className="text-xs font-extrabold text-gray-900 block mt-0.5 hover:text-pink-600 transition-colors"
              >
                +91 86104 48427
              </a>
            </div>
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <h4 className="text-xs font-bold text-pink-600">Timings</h4>
            </div>

            <div className="grid grid-cols-2 gap-2.5 pt-1">
              <div className="bg-pink-50/50 rounded-2xl p-3 border border-pink-100/60 flex items-center gap-2.5">
                <Sun className="w-5 h-5 text-pink-600 shrink-0" />
                <div>
                  <span className="text-[11px] font-bold text-gray-900 block leading-tight">Morning</span>
                  <span className="text-[10px] text-gray-600 font-medium block mt-0.5">8:00 AM – 1:00 PM</span>
                </div>
              </div>
              <div className="bg-pink-50/50 rounded-2xl p-3 border border-pink-100/60 flex items-center gap-2.5">
                <Moon className="w-5 h-5 text-pink-600 shrink-0" />
                <div>
                  <span className="text-[11px] font-bold text-gray-900 block leading-tight">Evening</span>
          ...
          {/* Map widget */}
          <div className="relative rounded-2xl overflow-hidden border border-gray-100 shadow-2xs bg-slate-100 mt-2">
            <div className="relative w-full h-36 bg-[#E8EDE9] p-3 overflow-hidden">
              ...
              <div className="absolute top-5 right-6 bg-white/95 backdrop-blur-xs px-2.5 py-1 rounded-full shadow-md border border-pink-100 flex items-center gap-1">
                ...
                <span className="text-[11px] font-extrabold text-pink-600">Thaai Clinic</span>
              </div>
            </div>
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="w-full bg-pink-600 hover:bg-pink-700 text-white py-2.5 px-4 font-extrabold text-xs flex items-center justify-center gap-2 transition-colors cursor-pointer"
            >
              <span>Get Directions</span>
              <div className="w-5 h-5 rounded-full bg-white text-pink-600 flex items-center justify-center">
                <Navigation className="w-3 h-3 fill-pink-600 stroke-none" />
              </div>
            </a>
          </div>
          ...
          <div className="flex items-start gap-3 pt-1">
            ...
            <div>
              <h4 className="text-xs font-bold text-pink-600">Nearby Landmark</h4>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium mt-0.5">
                Consult with Dr. Sakthimaindan at Thaai Clinic
              </p>
            </div>
          </div>
          ...
          <p className="text-xs text-gray-600 font-medium leading-relaxed">
            ...
            <a href="tel:+918610448427" className="font-extrabold text-pink-600 hover:underline">
              +91 86104 48427
            </a>{' '}
          ...
          <div className="flex gap-2.5 pt-1">
            <button
              ...
              className="flex-1 bg-pink-600 hover:bg-pink-700 text-white font-extrabold py-3 px-4 rounded-2xl text-xs shadow-md shadow-pink-200 transition-all active:scale-95 text-center cursor-pointer"
            >
              Book Visit Online
            </button>
            {onOpenWhatsApp && (
              <button
                ...
                className="bg-emerald-50 border border-emerald-200 text-emerald-700 hover:bg-emerald-100 font-bold py-3 px-4 rounded-2xl text-xs transition-colors cursor-pointer"
              >
                WhatsApp
              </button>
            )}
        ...
        <section
          ...
          className="bg-gradient-to-r from-pink-50 via-rose-50 to-pink-50 rounded-3xl p-4 border border-pink-100 shadow-2xs flex items-center justify-between relative overflow-hidden"
        >
          <div className="flex items-center gap-3 relative z-10">
            <div className="w-12 h-12 rounded-full bg-white border border-pink-100 flex items-center justify-center shrink-0 shadow-xs">
              <Heart className="w-6 h-6 fill-pink-600 stroke-none" />
            </div>
            <div>
              <p className="text-xs sm:text-sm font-extrabold text-pink-600 leading-snug">
                Thaai Clinic, Karaikal
              </p>
